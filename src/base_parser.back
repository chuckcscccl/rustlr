...
pub struct ZCRProduction<AT:Default,ET:Default>  // runtime rep of grammar rule
{
  ...
  pub Ruleaction : fn(&mut BaseParser<AT,ET>) -> AT, //parser as arg
}
...
/// by user programs.
pub struct BaseParser<AT:Default,ET:Default>  
{
...
impl<AT:Default,ET:Default> BaseParser<AT,ET>
{
    ...
    pub fn new(rlen:usize, slen:usize/*,tk:&'t mut dyn Tokenizer<'t,AT>*/) -> BaseParser<AT,ET>
...
extern crate rustlr;
use rustlr::{{Tokenizer,TerminalToken,BaseParser,ZCRProduction,Stateaction,decode_action}};\n")?;
    if self.Gmr.genlex {
...
use std::collections::{{HashMap,HashSet}};\n")?;
    ...
    write!(fd,"pub fn make_parser{}() -> BaseParser<{},{}>",&ltopt,absyn,extype)?; 
    write!(fd,"\n{{\n")?;
    // write code to pop stack, assign labels to variables.
    write!(fd," let mut parser1:BaseParser<{},{}> = BaseParser::new({},{});\n",absyn,extype,self.Gmr.Rules.len(),self.FSM.len())?;
...
/// A trait object that implements ErrReporter is expected by the [BaseParser::parse_core]
...
{
  fn err_reporter(&mut self, parser:&mut BaseParser<AT,ET>, lookahead:&TerminalToken<AT>, erropt:&Option<Stateaction>, tokenizer:& dyn Tokenizer<'_,AT>);
  fn report_err(&self, parser:&mut BaseParser<AT,ET>, msg:&str) { parser.report(msg) }
//  fn training_mode(&self, parser:&BaseParser<AT,ET>) -> bool {false}
//  fn interactive_mode(&self, parser:&BaseParser<AT,ET>) -> bool {false}
...
impl<AT:Default,ET:Default> BaseParser<AT,ET>
{
  ...
  pub fn parse_core<'u,'t:'u, TT:Tokenizer<'t,AT>>(&mut self, tokenizer:&'u mut TT, err_handler:&mut dyn ErrReporter<AT,ET>) -> AT
  {
    self.stack.clear();
    self.err_occurred = false;
    let mut result = AT::default();
  ...
  pub fn parse<'t, TT:Tokenizer<'t,AT>>(&mut self, tokenizer:&mut TT) -> AT
  {
  ...
  pub fn parse_train<'t, TT:Tokenizer<'t,AT>>(&mut self, tokenizer:&mut TT, parserfile:&str) -> AT
    {
  ...
  pub fn train_from_script<'t, TT:Tokenizer<'t,AT>>(&mut self, tokenizer:&mut TT,parserfile:&str, scriptfile:&str)
  {
