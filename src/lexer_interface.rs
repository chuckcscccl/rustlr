...
{
  /// this field must match the name of a terminal symbol of a grammar
  pub sym: &'t str,
  pub value: AT,
  pub line:usize,
  pub column:usize,
...
{
  /// retrieves the next Lextoken, or None at end-of-stream. 
  fn nextsym(&mut self) -> Option<TerminalToken<AT>>;
  /// returns the current line number.  The default implementation
  /// returns 0.
  fn linenum(&self) -> usize { 0 } // line number
  /// returns the current column (character position) on the current line.
...
impl<'t> StrTokenizer<'t>
{
   ...
   ///   ```ignore
   ///   let source = LexSource::new(source_path);
   ///   let mut tokenizer = StrTokenizer::from_source(&source);
   ///   ```
   pub fn from_source<'u:'t>(ls:&'t LexSource<'u>) ->StrTokenizer<'t>
   {
      let mut stk = StrTokenizer::new();
      stk.set_input(&ls.contents);
      stk
   }
}// impl StrTokenizer
