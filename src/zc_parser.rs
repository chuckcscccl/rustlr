...
pub struct ZCRProduction<AT:Default,ET:Default>  // runtime rep of grammar rule
{
  ...
  pub Ruleaction : fn(&mut ZCParser<'_,AT,ET>) -> AT, //parser as arg
}
...
impl<'t, AT:Default,ET:Default> ZCParser<'t, AT,ET>
{
    ...
    pub fn new(rlen:usize, slen:usize,tk:&'t mut dyn Tokenizer<'t,AT>) -> ZCParser<'t,AT,ET>
...
//    write!(fd,"   parser1.RSM[i].insert(SYMBOLS[k],decode_action(TABLE[i*{}+k]));\n }}}}\n",cols)?;
    ...
    ////// Augment!
    write!(fd,"fn load_extras<'parser_lt>(parser:&mut ZCParser<'parser_lt,{},{}>)\n{{\n",absyn,extype)?;
...
////////////////////////////////////////////////
  ...
  pub fn writelbaparser(&self, filename:&str)->Result<(),std::io::Error>
  {
    ...
    for ri in 0..rlen
    {
      ...
      let mut fndef = format!("fn _semaction_for_{}_<'parser_lt>(parser:&mut ZCParser<{},{}>) -> {} {{\n",ri,absyn,extype,rettype);
...
impl<'t,AT:Default,ET:Default> ZCParser<'t,AT,ET>
{
  ...
  /// interface.
  pub fn parse_core(&mut self, err_handler:&mut dyn ErrReporter<AT,ET>) -> AT
  {
    self.stack.clear();
