...
//! expects dynamic [Tokenizer] and [ErrHandler] trait-objects. 
...
use crate::{RProduction,RuntimeParser,StandardReporter};
...
/// by user programs.
pub struct ZCParser<'t, AT:Default,ET:Default>  
{
...
impl<'t, AT:Default,ET:Default> ZCParser<'t, AT,ET>
{
...
//         eprint!("ERROR on line {}, column {}:\n{}\n",self.linenum,self.column,tokenizer.current_line());         
  ...
  fn shift(&mut self, nextstate:usize, lookahead:TerminalToken<'t,AT>) -> TerminalToken<'t, AT>
  {
    ...
    fn reduce(&mut self, ri:&usize)
    {
       ...
       let val = (rulei.Ruleaction)(&mut dummy); // should be self
...
*/
    ...
    {
      self.parse_stdio_train(parserfile)
    }//parse_train
...
extern crate rustlr;
use rustlr::{{ZCParser,RProduction,Stateaction,decode_action}};\n")?;
    ...
    write!(fd,"pub fn make_parser() -> ZCParser<{},{}>",absyn,extype)?; 
    write!(fd,"\n{{\n")?;
  ...
  pub fn error_recover(&mut self, lookahead:&mut TerminalToken<'t,AT>) -> Option<Stateaction>
  {
    ...
    if self.Errsym.len()>0 {
      ...
      if let Some(Shift(i)) = erraction { // simulate shift errsym 
          ...
          while let None = self.RSM[*i].get(lookahead.sym) {
            ...
            *lookahead = if let Some(tok) = self.tokenizer.nextsym() {tok}
                else { TerminalToken::new("EOF",AT::default(),self.linenum,self.column) };
          }//while let
    ...
    if iserror(&erraction) && self.resynch.len()>0 {
      ...
      if lookahead.sym!="EOF" {
        ...
        self.linenum = lookahead.line; self.column = lookahead.column;
        //lookahead = self.nexttoken(); // skipp err-causing symbol
        *lookahead = if let Some(tok) = self.tokenizer.nextsym() {tok}
        else { TerminalToken::new("EOF",AT::default(),self.linenum,self.column) };
      }
...
/// A trait object that implements ErrHandler is expected by the [ZCParser::parse_core]
...
/// [ZCParser::parse_stdio], [ZCParser::parse_stdio_train] and
/// [ZCParser::train_from_script] functions.
...
impl<'t,AT:Default,ET:Default> ZCParser<'t,AT,ET>
{
  ...
  /// interface.
  pub fn parse_core(&mut self, err_handler:&mut dyn ErrHandler<AT,ET>) -> AT
  {
    self.stack.clear();
  ...
  pub fn parse_stdio(&mut self) -> AT
  {
  ...
  pub fn parse_stdio_train(&mut self, parserfile:&str) -> AT
    {
