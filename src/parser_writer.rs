...
// NEW APPROACH 0.4.13: let ast_writer decide if it's an lbox - always
        ...
        if self.Gmr.bumpast && boxedlabel {  // if bumpast and [x] label
           ...
           if self.Gmr.Rules[ri].autogenerated { //auto-generated type/action
           ...
           else {  // action not auto, must create LC
             stat = format!("let mut _{0}_ = if let RetTypeEnum::Enumvariant_{1}(_x_{1})=parser.popstack().value {{ _x_{1} }} else {{<{2}>::default()}};  let mut {0} = parser.lc({3},_{0}_);  ",poppedlab,&eindex,symtype,k-1);           
           }
        } // not bumpast
