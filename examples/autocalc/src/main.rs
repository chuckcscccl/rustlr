...
pub enum Env<'t> { // enviornment for evaluation
  Empty, 
...
pub fn eval<'t>(env:&Rc<Env<'t>>, exp:&Expr<'t>) -> Option<i64>
{
   match exp {
     ...
     Div(x,y) => eval(env,y).map(|yval|{if yval==0 {
          eprint!("Division by zero line {}, column {} ... ",y.line(),y.column());
	  None
         } else {eval(env,x).map(|xval|xval/yval)}
       }).flatten(),
     Let{let_var:x,init_value:e,let_body:b} =>
       eval(env,e).map(|ve|{ let newenv = push(x,ve,env); eval(&newenv,b) })
       .flatten(),
     _ => None,
   }//match
}//eval
