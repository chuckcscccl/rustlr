...
//println!("!!!!!etype: {:?}",&etype);
  ...
  // compile a variable, last flag indicates if index is known (x_5)
...
//println!("compiling define {}, var {}, current frame {}, structmap for {}",x,fvar,self.symbol_table.current,&xvar);  
        ...
        func.add_inst(Store(fvartype.clone(),fvardest,Register(r4),None))
      } // store copies into closure of xvar function
...
//    fargs.push((closurearg,str24::from(&format!("{}_self",&fname)))); //self
    ...
    /*
    if let Novalue = &bdest { // void function
      newfunc.add_inst(Ret(Basic("i32"),Iconst(0)));
    }
    else {
      newfunc.add_inst(Ret(rettype,bdest));
    }
