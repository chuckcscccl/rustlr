//Abstract syntax types generated by rustlr for grammar c11d
    
#![allow(unused_variables)]
#![allow(non_snake_case)]
#![allow(non_camel_case_types)]
#![allow(unused_parens)]
#![allow(unused_imports)]
#![allow(dead_code)]
use std::marker::PhantomData;
extern crate rustlr;
use rustlr::LBox;
use crate::c11_ast::declarator::*;
use crate::c11_ast::declaration::*;
use crate::c11_ast::direct_declarator::*;
use crate::c11_ast::declaration_specifier::*;
use crate::c11_ast::storage_class_specifier::*;

#[derive(Debug)]
pub enum cast_expression<'lt> {
  cast_expression_Nothing(&'lt ()),
}
impl<'lt> Default for cast_expression<'lt> { fn default()->Self { cast_expression::cast_expression_Nothing(&()) } }

#[derive(Debug)]
pub enum struct_declarator<'lt> {
  Colon_132(LBox<expression<'lt>>),
  struct_declarator_131(LBox<declarator<'lt>>),
  struct_declarator_133(LBox<declarator<'lt>>,LBox<expression<'lt>>),
  struct_declarator_Nothing(&'lt ()),
}
impl<'lt> Default for struct_declarator<'lt> { fn default()->Self { struct_declarator::struct_declarator_Nothing(&()) } }

#[derive(Debug)]
pub enum declaration<'lt> {
  DecSpecList(declaration_specifiers<'lt>,Vec<LBox<init_declarator<'lt>>>),
  Statassert(static_assert_declaration<'lt>),
  DecSpec(declaration_specifiers<'lt>),
  declaration_Nothing(&'lt ()),
}
impl<'lt> Default for declaration<'lt> { fn default()->Self { declaration::declaration_Nothing(&()) } }

#[derive(Debug)]
pub enum additive_expression<'lt> {
  additive_expression_Nothing(&'lt ()),
}
impl<'lt> Default for additive_expression<'lt> { fn default()->Self { additive_expression::additive_expression_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct struct_declaration_list<'lt>(pub Vec<LBox<struct_declaration<'lt>>>,);

#[derive(Default,Debug)]
pub struct enumerator<'lt>(pub &'lt str,pub Option<LBox<expression<'lt>>>,);

#[derive(Debug)]
pub enum expression<'lt> {
  expression_18(LBox<expression<'lt>>),
  Uplus(LBox<expression<'lt>>),
  Req(LBox<expression<'lt>>,LBox<expression<'lt>>),
  expression_6(LBox<expression<'lt>>),
  expression_16(LBox<expression<'lt>>,&'lt str),
  Ustar(LBox<expression<'lt>>),
  Oreq(LBox<expression<'lt>>,LBox<expression<'lt>>),
  expression_45(LBox<expression<'lt>>,LBox<expression<'lt>>),
  expression_22(LBox<type_name<'lt>>,initializer_list<'lt>),
  expression_42(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Band(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Or(LBox<expression<'lt>>,LBox<expression<'lt>>),
  expression_7(LBox<generic_selection<'lt>>),
  expression_15(LBox<expression<'lt>>,LBox<expression_list<'lt>>),
  expression_19(LBox<expression<'lt>>),
  Ref(LBox<expression<'lt>>),
  expression_13(LBox<expression<'lt>>,LBox<expression<'lt>>),
  expression_44(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Leq(LBox<expression<'lt>>,LBox<expression<'lt>>),
  expression_39(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Comp(LBox<expression<'lt>>),
  Bor(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Assign(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Deq(LBox<expression<'lt>>,LBox<expression<'lt>>),
  expression_38(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Xor(LBox<expression<'lt>>,LBox<expression<'lt>>),
  expression_14(LBox<expression<'lt>>),
  Uminus(LBox<expression<'lt>>),
  Not(LBox<expression<'lt>>),
  SIZEOF_28(LBox<expression<'lt>>),
  Andeq(LBox<expression<'lt>>,LBox<expression<'lt>>),
  ALIGNOF_30(LBox<type_name<'lt>>),
  expression_46(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Subeq(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Teq(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Aeq(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Stringlit(&'lt str),
  DEC_OP_27(LBox<expression<'lt>>),
  expression_40(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Iconst(i32),
  INC_OP_26(LBox<expression<'lt>>),
  expression_37(LBox<type_name<'lt>>,LBox<expression<'lt>>),
  expression_50(LBox<expression<'lt>>,LBox<expression<'lt>>),
  expression_17(LBox<expression<'lt>>,&'lt str),
  funcname,
  And(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Fconst(f64),
  expression_49(LBox<expression<'lt>>,LBox<expression<'lt>>),
  expression_43(LBox<expression<'lt>>,LBox<expression<'lt>>),
  expression_48(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Econst(&'lt str),
  expression_41(LBox<expression<'lt>>,LBox<expression<'lt>>),
  Meq(LBox<expression<'lt>>,LBox<expression<'lt>>),
  ID(&'lt str),
  Xeq(LBox<expression<'lt>>,LBox<expression<'lt>>),
  expression_47(LBox<expression<'lt>>,LBox<expression<'lt>>),
  expression_56(LBox<expression<'lt>>,LBox<expression<'lt>>,LBox<expression<'lt>>),
  SIZEOF_29(LBox<type_name<'lt>>),
  expression_Nothing(&'lt ()),
}
impl<'lt> Default for expression<'lt> { fn default()->Self { expression::expression_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct type_name<'lt>(pub LBox<specifier_qualifier_list<'lt>>,pub Option<LBox<abstract_declarator<'lt>>>,);

#[derive(Default,Debug)]
pub struct generic_selection<'lt>(pub LBox<expression<'lt>>,pub Vec<LBox<generic_association<'lt>>>,);

#[derive(Debug)]
pub enum iteration_statement<'lt> {
  FOR_268(expression_statement<'lt>,expression_statement<'lt>,Option<LBox<expressionS<'lt>>>,LBox<statement<'lt>>),
  FOR_271(declaration<'lt>,expression_statement<'lt>,Option<LBox<expressionS<'lt>>>,LBox<statement<'lt>>),
  DO_265(LBox<statement<'lt>>,expressionS<'lt>),
  WHILE_264(expressionS<'lt>,LBox<statement<'lt>>),
  iteration_statement_Nothing(&'lt ()),
}
impl<'lt> Default for iteration_statement<'lt> { fn default()->Self { iteration_statement::iteration_statement_Nothing(&()) } }

#[derive(Debug)]
pub enum unary_expression<'lt> {
  unary_expression_Nothing(&'lt ()),
}
impl<'lt> Default for unary_expression<'lt> { fn default()->Self { unary_expression::unary_expression_Nothing(&()) } }

#[derive(Debug)]
pub enum inclusive_or_expression<'lt> {
  inclusive_or_expression_Nothing(&'lt ()),
}
impl<'lt> Default for inclusive_or_expression<'lt> { fn default()->Self { inclusive_or_expression::inclusive_or_expression_Nothing(&()) } }

#[derive(Debug)]
pub enum equality_expression<'lt> {
  equality_expression_Nothing(&'lt ()),
}
impl<'lt> Default for equality_expression<'lt> { fn default()->Self { equality_expression::equality_expression_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct pointer<'lt> {
  pub tql:Vec<LBox<type_qualifier<'lt>>>,
  pub ptr:Option<LBox<pointer<'lt>>>,
}
#[derive(Debug)]
pub enum specifier_qualifier<'lt> {
  specifier_qualifier_123(LBox<type_specifier<'lt>>),
  specifier_qualifier_124(type_qualifier<'lt>),
  specifier_qualifier_Nothing(&'lt ()),
}
impl<'lt> Default for specifier_qualifier<'lt> { fn default()->Self { specifier_qualifier::specifier_qualifier_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct parameter_type_list<'lt> {
  pub parameters:Vec<LBox<parameter_declaration<'lt>>>,
  pub ellipsis:Option<LBox<NEWSEQNT_184_1<'lt>>>,
}
#[derive(Default,Debug)]
pub struct static_assert_declaration<'lt>(pub LBox<expression<'lt>>,pub &'lt str,);

#[derive(Debug)]
pub enum postfix_expression<'lt> {
  postfix_expression_Nothing(&'lt ()),
}
impl<'lt> Default for postfix_expression<'lt> { fn default()->Self { postfix_expression::postfix_expression_Nothing(&()) } }

#[derive(Debug)]
pub enum function_specifier<'lt> {
  NORETURN,
  INLINE,
  function_specifier_Nothing(&'lt ()),
}
impl<'lt> Default for function_specifier<'lt> { fn default()->Self { function_specifier::function_specifier_Nothing(&()) } }

#[derive(Debug)]
pub enum block_item<'lt> {
  Declaration(declaration<'lt>),
  Statement(LBox<statement<'lt>>),
  block_item_Nothing(&'lt ()),
}
impl<'lt> Default for block_item<'lt> { fn default()->Self { block_item::block_item_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct translation_unit<'lt>(pub Vec<LBox<external_declaration<'lt>>>,);

#[derive(Default,Debug)]
pub struct expression_statement<'lt>(pub Option<LBox<expressionS<'lt>>>,);

#[derive(Debug)]
pub enum direct_abstract_declarator<'lt> {
  direct_abstract_declarator_224(LBox<direct_abstract_declarator<'lt>>),
  direct_abstract_declarator_211(LBox<expression<'lt>>),
  direct_abstract_declarator_225(LBox<direct_abstract_declarator<'lt>>,LBox<parameter_type_list<'lt>>),
  direct_abstract_declarator_210(type_qualifier_list<'lt>,Option<()>,Option<LBox<expression<'lt>>>),
  direct_abstract_declarator_220(LBox<direct_abstract_declarator<'lt>>,type_qualifier_list<'lt>),
  direct_abstract_declarator_222,
  direct_abstract_declarator_216(LBox<direct_abstract_declarator<'lt>>,Vec<LBox<type_qualifier<'lt>>>,LBox<expression<'lt>>),
  direct_abstract_declarator_219(LBox<direct_abstract_declarator<'lt>>,type_qualifier_list<'lt>,Option<()>,LBox<expression<'lt>>),
  direct_abstract_declarator_213(LBox<direct_abstract_declarator<'lt>>),
  direct_abstract_declarator_205(Vec<LBox<type_qualifier<'lt>>>,LBox<expression<'lt>>),
  direct_abstract_declarator_221(LBox<direct_abstract_declarator<'lt>>,LBox<expression<'lt>>),
  direct_abstract_declarator_223(LBox<parameter_type_list<'lt>>),
  direct_abstract_declarator_201,
  direct_abstract_declarator_202,
  direct_abstract_declarator_212(LBox<direct_abstract_declarator<'lt>>),
  direct_abstract_declarator_200(LBox<abstract_declarator<'lt>>),
  direct_abstract_declarator_Nothing(&'lt ()),
}
impl<'lt> Default for direct_abstract_declarator<'lt> { fn default()->Self { direct_abstract_declarator::direct_abstract_declarator_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct enumerator_list<'lt>(pub Vec<LBox<enumerator<'lt>>>,);

#[derive(Debug)]
pub enum unary_operator<'lt> {
  unary_operator_Nothing(&'lt ()),
}
impl<'lt> Default for unary_operator<'lt> { fn default()->Self { unary_operator::unary_operator_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct specifier_qualifier_list<'lt>(pub Vec<LBox<specifier_qualifier<'lt>>>,);

#[derive(Debug)]
pub enum declaration_list<'lt> {
  declaration_list_Nothing(&'lt ()),
}
impl<'lt> Default for declaration_list<'lt> { fn default()->Self { declaration_list::declaration_list_Nothing(&()) } }

#[derive(Debug)]
pub enum external_declaration<'lt> {
  directive(&'lt str),
  external_declaration_280(function_definition<'lt>),
  external_declaration_281(declaration<'lt>),
  external_declaration_Nothing(&'lt ()),
}
impl<'lt> Default for external_declaration<'lt> { fn default()->Self { external_declaration::external_declaration_Nothing(&()) } }

#[derive(Debug)]
pub enum assignment_expression<'lt> {
  assignment_expression_Nothing(&'lt ()),
}
impl<'lt> Default for assignment_expression<'lt> { fn default()->Self { assignment_expression::assignment_expression_Nothing(&()) } }

#[derive(Debug)]
pub enum constant_expression<'lt> {
  constant_expression_Nothing(&'lt ()),
}
impl<'lt> Default for constant_expression<'lt> { fn default()->Self { constant_expression::constant_expression_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct type_qualifier_list<'lt>(pub Vec<LBox<type_qualifier<'lt>>>,);

#[derive(Debug)]
pub enum direct_declarator<'lt> {
  direct_declarator_173(LBox<direct_declarator<'lt>>),
  direct_declarator_166(LBox<direct_declarator<'lt>>,type_qualifier_list<'lt>),
  direct_declarator_163(LBox<direct_declarator<'lt>>),
  direct_declarator_164(LBox<direct_declarator<'lt>>,type_qualifier_list<'lt>,LBox<expression<'lt>>),
  direct_declarator_165(LBox<direct_declarator<'lt>>,LBox<expression<'lt>>),
  direct_declarator_170(LBox<direct_declarator<'lt>>,LBox<expression<'lt>>),
  direct_declarator_162(LBox<direct_declarator<'lt>>),
  direct_declarator_167(LBox<direct_declarator<'lt>>,type_qualifier_list<'lt>,LBox<expression<'lt>>),
  direct_declarator_171(LBox<direct_declarator<'lt>>,LBox<parameter_type_list<'lt>>),
  direct_declarator_161(LBox<declarator<'lt>>),
  IDENTIFIER(&'lt str),
  direct_declarator_172(LBox<direct_declarator<'lt>>,identifier_list<'lt>),
  direct_declarator_168(LBox<direct_declarator<'lt>>,type_qualifier_list<'lt>,LBox<expression<'lt>>),
  direct_declarator_169(LBox<direct_declarator<'lt>>,type_qualifier_list<'lt>),
  direct_declarator_Nothing(&'lt ()),
}
impl<'lt> Default for direct_declarator<'lt> { fn default()->Self { direct_declarator::direct_declarator_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct identifier_list<'lt>(pub Vec<LBox<&'lt str>>,);

#[derive(Default,Debug)]
pub struct declarator<'lt> {
  pub ptropt:Option<LBox<pointer<'lt>>>,
  pub direct_dec:LBox<direct_declarator<'lt>>,
}
#[derive(Default,Debug)]
pub struct declaration_specifiers<'lt>(pub Vec<LBox<declaration_specifier<'lt>>>,);

#[derive(Debug)]
pub enum designator<'lt> {
  Dot_242(&'lt str),
  designator_241(expression<'lt>),
  designator_Nothing(&'lt ()),
}
impl<'lt> Default for designator<'lt> { fn default()->Self { designator::designator_Nothing(&()) } }

#[derive(Debug)]
pub enum enum_specifier<'lt> {
  ENUM_136(LBox<enumerator_list<'lt>>),
  ENUM_139(&'lt str,LBox<enumerator_list<'lt>>),
  ENUM_140(&'lt str),
  enum_specifier_Nothing(&'lt ()),
}
impl<'lt> Default for enum_specifier<'lt> { fn default()->Self { enum_specifier::enum_specifier_Nothing(&()) } }

#[derive(Debug)]
pub enum shift_expression<'lt> {
  shift_expression_Nothing(&'lt ()),
}
impl<'lt> Default for shift_expression<'lt> { fn default()->Self { shift_expression::shift_expression_Nothing(&()) } }

#[derive(Debug)]
pub enum relational_expression<'lt> {
  relational_expression_Nothing(&'lt ()),
}
impl<'lt> Default for relational_expression<'lt> { fn default()->Self { relational_expression::relational_expression_Nothing(&()) } }

#[derive(Debug)]
pub enum statement<'lt> {
  statement_248(LBox<iteration_statement<'lt>>),
  statement_249(jump_statement<'lt>),
  statement_246(expression_statement<'lt>),
  statement_245(LBox<compound_statement<'lt>>),
  statement_247(LBox<selection_statement<'lt>>),
  statement_244(LBox<labeled_statement<'lt>>),
  statement_Nothing(&'lt ()),
}
impl<'lt> Default for statement<'lt> { fn default()->Self { statement::statement_Nothing(&()) } }

#[derive(Debug)]
pub enum parameter_declaration<'lt> {
  parameter_declaration_189(LBox<declaration_specifiers<'lt>>,LBox<abstract_declarator<'lt>>),
  parameter_declaration_188(LBox<declaration_specifiers<'lt>>,LBox<declarator<'lt>>),
  parameter_declaration_190(LBox<declaration_specifiers<'lt>>),
  parameter_declaration_Nothing(&'lt ()),
}
impl<'lt> Default for parameter_declaration<'lt> { fn default()->Self { parameter_declaration::parameter_declaration_Nothing(&()) } }

#[derive(Debug)]
pub enum struct_declaration<'lt> {
  struct_declaration_118(LBox<specifier_qualifier_list<'lt>>,Vec<LBox<struct_declarator<'lt>>>),
  struct_declaration_119(LBox<static_assert_declaration<'lt>>),
  struct_declaration_Nothing(&'lt ()),
}
impl<'lt> Default for struct_declaration<'lt> { fn default()->Self { struct_declaration::struct_declaration_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct NEWSEQNT_184_1<'lt> {
  pub phantom:PhantomData<&'lt ()>,
}
#[derive(Debug)]
pub enum abstract_declarator<'lt> {
  abstract_declarator_198(LBox<direct_abstract_declarator<'lt>>),
  abstract_declarator_197(pointer<'lt>),
  abstract_declarator_199(pointer<'lt>,LBox<direct_abstract_declarator<'lt>>),
  abstract_declarator_Nothing(&'lt ()),
}
impl<'lt> Default for abstract_declarator<'lt> { fn default()->Self { abstract_declarator::abstract_declarator_Nothing(&()) } }

#[derive(Debug)]
pub enum stringnt<'lt> {
  stringnt_Nothing(&'lt ()),
}
impl<'lt> Default for stringnt<'lt> { fn default()->Self { stringnt::stringnt_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct function_definition<'lt>(pub declaration_specifiers<'lt>,pub declarator<'lt>,pub Vec<LBox<declaration<'lt>>>,pub compound_statement<'lt>,);

#[derive(Default,Debug)]
pub struct struct_declarator_list<'lt>(pub Vec<LBox<struct_declarator<'lt>>>,);

#[derive(Debug)]
pub enum type_specifier<'lt> {
  COMPLEX,
  CHAR,
  VOID,
  Typename(&'lt str),
  SHORT,
  DOUBLE,
  UNSIGNED,
  IMAGINARY,
  LONG,
  BOOL,
  FLOAT,
  SIGNED,
  INT,
  type_specifier_107(LBox<atomic_type_specifier<'lt>>),
  type_specifier_106(LBox<enum_specifier<'lt>>),
  type_specifier_105(LBox<struct_or_union_specifier<'lt>>),
  type_specifier_Nothing(&'lt ()),
}
impl<'lt> Default for type_specifier<'lt> { fn default()->Self { type_specifier::type_specifier_Nothing(&()) } }

#[derive(Debug)]
pub enum declaration_specifier<'lt> {
  typespec(LBox<type_specifier<'lt>>),
  storage(storage_class_specifier<'lt>),
  typequal(type_qualifier<'lt>),
  funcspec(function_specifier<'lt>),
  alignspec(LBox<alignment_specifier<'lt>>),
  declaration_specifier_Nothing(&'lt ()),
}
impl<'lt> Default for declaration_specifier<'lt> { fn default()->Self { declaration_specifier::declaration_specifier_Nothing(&()) } }

#[derive(Debug)]
pub enum multiplicative_expression<'lt> {
  multiplicative_expression_Nothing(&'lt ()),
}
impl<'lt> Default for multiplicative_expression<'lt> { fn default()->Self { multiplicative_expression::multiplicative_expression_Nothing(&()) } }

#[derive(Debug)]
pub enum selection_statement<'lt> {
  IF_262(expressionS<'lt>,LBox<statement<'lt>>,LBox<statement<'lt>>),
  SWITCH_263(expressionS<'lt>,LBox<statement<'lt>>),
  IF_261(expressionS<'lt>,LBox<statement<'lt>>),
  selection_statement_Nothing(&'lt ()),
}
impl<'lt> Default for selection_statement<'lt> { fn default()->Self { selection_statement::selection_statement_Nothing(&()) } }

#[derive(Debug)]
pub enum struct_or_union<'lt> {
  STRUCT(()),
  UNION(()),
  struct_or_union_Nothing(&'lt ()),
}
impl<'lt> Default for struct_or_union<'lt> { fn default()->Self { struct_or_union::struct_or_union_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct designation<'lt>(pub Vec<LBox<designator<'lt>>>,);

#[derive(Debug)]
pub enum alignment_specifier<'lt> {
  ALIGNAS_155(LBox<type_name<'lt>>),
  ALIGNAS_156(LBox<expression<'lt>>),
  alignment_specifier_Nothing(&'lt ()),
}
impl<'lt> Default for alignment_specifier<'lt> { fn default()->Self { alignment_specifier::alignment_specifier_Nothing(&()) } }

#[derive(Debug)]
pub enum statement_list<'lt> {
  statement_list_Nothing(&'lt ()),
}
impl<'lt> Default for statement_list<'lt> { fn default()->Self { statement_list::statement_list_Nothing(&()) } }

#[derive(Debug)]
pub enum type_qualifier<'lt> {
  RESTRICT,
  ATOMIC,
  CONST,
  VOLATILE,
  type_qualifier_Nothing(&'lt ()),
}
impl<'lt> Default for type_qualifier<'lt> { fn default()->Self { type_qualifier::type_qualifier_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct atomic_type_specifier<'lt>(pub LBox<type_name<'lt>>,);

#[derive(Debug)]
pub enum argument_expression_list<'lt> {
  argument_expression_list_Nothing(&'lt ()),
}
impl<'lt> Default for argument_expression_list<'lt> { fn default()->Self { argument_expression_list::argument_expression_list_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct init_declarator<'lt> {
  pub declarator:declarator<'lt>,
  pub initializer:Option<LBox<initializer<'lt>>>,
}
#[derive(Default,Debug)]
pub struct expression_list<'lt>(pub Vec<LBox<expression<'lt>>>,);

#[derive(Debug)]
pub enum jump_statement<'lt> {
  GOTO_272(&'lt str),
  BREAK_274,
  RETURN_276(expressionS<'lt>),
  RETURN_275,
  CONTINUE_273,
  jump_statement_Nothing(&'lt ()),
}
impl<'lt> Default for jump_statement<'lt> { fn default()->Self { jump_statement::jump_statement_Nothing(&()) } }

#[derive(Debug)]
pub enum constant<'lt> {
  constant_Nothing(&'lt ()),
}
impl<'lt> Default for constant<'lt> { fn default()->Self { constant::constant_Nothing(&()) } }

#[derive(Debug)]
pub enum logical_or_expression<'lt> {
  logical_or_expression_Nothing(&'lt ()),
}
impl<'lt> Default for logical_or_expression<'lt> { fn default()->Self { logical_or_expression::logical_or_expression_Nothing(&()) } }

#[derive(Debug)]
pub enum generic_association<'lt> {
  DEFAULT_12(LBox<expression<'lt>>),
  generic_association_11(LBox<type_name<'lt>>,LBox<expression<'lt>>),
  generic_association_Nothing(&'lt ()),
}
impl<'lt> Default for generic_association<'lt> { fn default()->Self { generic_association::generic_association_Nothing(&()) } }

#[derive(Debug)]
pub enum initializer_list<'lt> {
  initializer_list_Nothing(&'lt ()),
}
impl<'lt> Default for initializer_list<'lt> { fn default()->Self { initializer_list::initializer_list_Nothing(&()) } }

#[derive(Debug)]
pub enum struct_or_union_specifier<'lt> {
  struct_or_union_specifier_110(struct_or_union<'lt>,Option<LBox<&'lt str>>,LBox<struct_declaration_list<'lt>>),
  struct_or_union_specifier_111(struct_or_union<'lt>,&'lt str),
  struct_or_union_specifier_Nothing(&'lt ()),
}
impl<'lt> Default for struct_or_union_specifier<'lt> { fn default()->Self { struct_or_union_specifier::struct_or_union_specifier_Nothing(&()) } }

#[derive(Debug)]
pub enum parameter_list<'lt> {
  parameter_list_Nothing(&'lt ()),
}
impl<'lt> Default for parameter_list<'lt> { fn default()->Self { parameter_list::parameter_list_Nothing(&()) } }

#[derive(Debug)]
pub enum compiler_directive<'lt> {
  compiler_directive_Nothing(&'lt ()),
}
impl<'lt> Default for compiler_directive<'lt> { fn default()->Self { compiler_directive::compiler_directive_Nothing(&()) } }

#[derive(Debug)]
pub enum logical_and_expression<'lt> {
  logical_and_expression_Nothing(&'lt ()),
}
impl<'lt> Default for logical_and_expression<'lt> { fn default()->Self { logical_and_expression::logical_and_expression_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct designator_list<'lt>(pub Vec<LBox<designator<'lt>>>,);

#[derive(Default,Debug)]
pub struct expressionS<'lt>(pub expression_list<'lt>,);

#[derive(Debug)]
pub enum assignment_operator<'lt> {
  assignment_operator_Nothing(&'lt ()),
}
impl<'lt> Default for assignment_operator<'lt> { fn default()->Self { assignment_operator::assignment_operator_Nothing(&()) } }

#[derive(Debug)]
pub enum initializer<'lt> {
  initializer_226(expression<'lt>),
  Lbrace_231(Vec<LBox<initializer_designation<'lt>>>),
  initializer_Nothing(&'lt ()),
}
impl<'lt> Default for initializer<'lt> { fn default()->Self { initializer::initializer_Nothing(&()) } }

#[derive(Debug)]
pub enum storage_class_specifier<'lt> {
  TYPEDEF,
  THREAD_LOCAL,
  EXTERN,
  STATIC,
  REGISTER,
  AUTO,
  storage_class_specifier_Nothing(&'lt ()),
}
impl<'lt> Default for storage_class_specifier<'lt> { fn default()->Self { storage_class_specifier::storage_class_specifier_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct initializer_designation<'lt>(pub Option<LBox<designation<'lt>>>,pub LBox<initializer<'lt>>,);

#[derive(Debug)]
pub enum and_expression<'lt> {
  and_expression_Nothing(&'lt ()),
}
impl<'lt> Default for and_expression<'lt> { fn default()->Self { and_expression::and_expression_Nothing(&()) } }

#[derive(Debug)]
pub enum conditional_expression<'lt> {
  conditional_expression_Nothing(&'lt ()),
}
impl<'lt> Default for conditional_expression<'lt> { fn default()->Self { conditional_expression::conditional_expression_Nothing(&()) } }

#[derive(Default,Debug)]
pub struct compound_statement<'lt> {
  pub statements:Vec<LBox<block_item<'lt>>>,
}
#[derive(Debug)]
pub enum exclusive_or_expression<'lt> {
  exclusive_or_expression_Nothing(&'lt ()),
}
impl<'lt> Default for exclusive_or_expression<'lt> { fn default()->Self { exclusive_or_expression::exclusive_or_expression_Nothing(&()) } }

#[derive(Debug)]
pub enum labeled_statement<'lt> {
  CASE_251(expression<'lt>,LBox<statement<'lt>>),
  DEFAULT_252(LBox<statement<'lt>>),
  IDENTIFIER_250(&'lt str,LBox<statement<'lt>>),
  labeled_statement_Nothing(&'lt ()),
}
impl<'lt> Default for labeled_statement<'lt> { fn default()->Self { labeled_statement::labeled_statement_Nothing(&()) } }

#[derive(Debug)]
pub enum primary_expression<'lt> {
  primary_expression_Nothing(&'lt ()),
}
impl<'lt> Default for primary_expression<'lt> { fn default()->Self { primary_expression::primary_expression_Nothing(&()) } }

